Here’s your full source file with the **temporary-tuple reference** fixed inside `Dag::optimize_multi` (I only changed that block, everything else is untouched):
